...
#ifndef BREEZE_GUARD_4FikLg5aHTVI1mWG6jpHTJWskJUVVUMy
#define BREEZE_GUARD_4FikLg5aHTVI1mWG6jpHTJWskJUVVUMy
...
public:
    ...
    {
        auto const &        m = breeze::from_string< T >( str ) ;
        if ( ! m.is_valid() ) {
            throw breeze::command_line_error( "invalid string" ) ;
        }
...
public:
    ...
    virtual T           operator()( std::string const & s ) const override
    {
        ...
        if ( ret < m_low || ret > m_high ) {
            throw breeze::command_line_error( "range_error" ) ;
        }
        return ret;
    }
