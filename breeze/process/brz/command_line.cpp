    ...
    for ( int i = 1 ; i < argc ; ++ i ) {
        ...
        if ( begins_with( argv[i], dashdash ) ) {
            char const * const  p = std::strchr( argv[ i ] +
                                            ( breeze::signed_count( dashdash ) - 1 ), '=' ) ;
            if ( p != nullptr ) {
                std::string const   name( argv[ i ] + ( breeze::signed_count(dashdash) - 1 ), p ) ;
                std::string const   value( p + 1 ) ;
                set_option( name, value ) ;
            } else {
                std::string name( argv[ i ] + ( breeze::signed_count( dashdash ) - 1 ) ) ;
                if ( m_options.count( name ) == 0 ) {
...
command_line::usage() const
{
    ...
    for ( std::size_t i = 0 ; i < m_ordered.size() ; ++ i ) {
        max_width = std::max BREEZE_PREVENT_MACRO_EXPANSION
