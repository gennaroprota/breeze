...
#ifndef BREEZE_GUARD_Z7sZqZ1T5DzTJ6V6zdst2zbGcymbnLM7
#define BREEZE_GUARD_Z7sZqZ1T5DzTJ6V6zdst2zbGcymbnLM7
...
//!     then <tt>max_for_subrange(5, RAND_MAX)</tt> returns
//!     <tt>32765</tt>.
//!
...
//!     than discarding anything greater than five.
//!
//!     Why that value? The idea, of course, is that there are
//!     <tt>32766</tt> integers in <tt>[0, 32765]</tt>, and that is a
//!     multiple of six (the number of integers in <tt>[0, 5]</tt>). So,
//!     the remainders <tt>0</tt>, <tt>1</tt>, <tt>2</tt>, <tt>3</tt>,
//!     <tt>4</tt> and <tt>5</tt> all appear with the same probability
//!     (if the source is not biased). If we took the whole range then,
//!     of course, <tt>0</tt> and <tt>1</tt> would be more frequent than
//!     the others.
//!
//!     With respect to just discarding anything greater than
//!     <tt>x</tt>:
//!
//!      - For pseudo-random numbers, this basically saves execution
//!        time.
//!
//!      - For true random numbers, it also avoids needlessly discarding
